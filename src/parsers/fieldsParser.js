 ...
 * @return {Object}
 */
export default function (sysVals) {
  return (data, props = { include: 10 }) => {
    /**
    ...
    function parseFields(fieldsObject, sys, objectRef = {}, depth = 0) {
      ...
      if (!!contentTypeId) {
        ...
        if (sysVals && typeof sysVals === 'object') {
          Object.keys(sysVals).forEach(key => {
            const sysValue = get(sys, sysVals[key]);

            if (typeof sysValue !== 'undefined') {
              objectRefClone[key] = sysValue;
            }
          });
        }
        /* eslint-enable */
